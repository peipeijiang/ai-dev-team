...
watch(() => props.modelValue, (val) => {
  visible.value = val;
  if (val && props.type) {
    if (['character', 'scene', 'prop'].includes(props.type)) {
       // Map singular to plural if needed, or just match tab names
       // Tabs are: all, characters, scenes, props
       if (props.type === 'character') activeTab.value = 'characters';
       else if (props.type === 'scene') activeTab.value = 'scenes';
       else if (props.type === 'prop') activeTab.value = 'props';
       else activeTab.value = props.type;
    } else {
       activeTab.value = 'all';
    }
  }
