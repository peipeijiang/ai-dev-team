...
if os.path.exists(web_dir):
    ...
    # 尝试加载构建后的现代前端
    modern_web_dist = os.path.join(os.path.dirname(os.path.dirname(__file__)), "apps", "web-modern", "dist")
    if os.path.exists(modern_web_dist):
        app.mount("/", StaticFiles(directory=modern_web_dist, html=True), name="web-modern")
        print(f"Serving modern frontend from {modern_web_dist}")
    # 尝试加载旧的前端
    else:
        old_web_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "apps", "web")
        if os.path.exists(old_web_dir):
            app.mount("/", StaticFiles(directory=old_web_dir, html=True), name="web")
            print(f"Serving legacy frontend from {old_web_dir}")
        else:
